[bits 16]           ;以16位的实模式来运行，也就是8086 CPU的工作模式
[org 0x7c00]        ; 此处的 org 与关键字 vstart 同理,可以理解为设当前程序开始处为基址，自此以后的地址都以此来计算偏移量，
                    ; 设置值为0x7c00,是因为 bios 查找到有效mbr后，  会 jmp 0x7c00, 将 ip 指向此处
					;历史原因：当时内存为32KB,为了找到一块不被覆盖和空闲的空间，采取最后1KB 空间来存放，所以使用了7C00 作为开始，（7c00到8000正好为最后1kB空间)

start:              ;  instruction 执行的入口 

    xor ax,ax           ; 通过 xor  设置 ax 为0,然后依以将0赋值于 ds,es 寄存器
    mov ds,ax            
    mov es,ax           
    mov bx,0x8000

    mov ax,0x13         ; 设置bois 的中断向量子程序号13，13对应的子程序为清屏操作 
    int 0x10            ;调用 bois 的中断，中断向量为ox10 0x10中断号为显示卡中断 
